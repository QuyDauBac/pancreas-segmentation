...
1. **D đưa cho A:** ảnh xám, kiểu `uint8`, giá trị `0–255` (D đã HU windowing)
2. **A đưa cho B:** ảnh xám, kiểu `uint8`, giá trị `0–255`
3. **B đưa cho C/D:** mask nhị phân, giá trị `0` và `1` (KHÔNG phải 0/255)
...
|---|---|---|---|
| A | `preprocess.py` | Khử nhiễu (median/gaussian), Histogram Equalization, (tùy chọn) crop giữa | C1 + 3.2–3.3 |
| B | `segment.py` | Global/Otsu/Adaptive threshold, Morphology (opening) | C2 (Related Works) + 3.4–3.5 |
| C (trưởng nhóm) | `evaluate.py`, `main.py` | Dice/IoU, tích hợp, ghép & định dạng báo cáo, điều phối | C4.2 + C5 + front matter |
| D | `data_loader.py`, `experiments.py`, `app.py` | HU windowing + đọc dữ liệu, so sánh ngưỡng + ablation, demo Streamlit | C4.1 + C4.3 |
